...
func main() {
	...
	// TODO parse Link to get 'next' link
    ...
    for k := range mapLocker.Lmap { //each language
        if print {
            fmt.Printf("%s: %d lines\n", k, mapLocker.Lmap[k]) // language name : language number lines
        }
        for i := 0; i < nb; i++ {   // each repository
            for kk := range repositories[i].Languages { // each language of each repository
                if kk == k { // test if the repository has the concerning language
                    if print {
                        fmt.Printf("\t - %s : ", repositories[i].Html_url)	// repo name
                        fmt.Printf("%d lines\n", repositories[i].Languages[kk])	// repo number lines
                    }
                }
            }
        }
